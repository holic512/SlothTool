 ...
 * @description 提供官方插件在线/离线安装、归档、更新、卸载、运行、别名迁移、最近运行记录、自更新和全量数据删除等无 UI 生命周期能力，并统一接入代理与 GitHub 镜像配置。
 * @logic 1. 用 reporter 事件输出进度；2. 统一处理 GitHub Release、离线归档、遗留 npm 来源与 SlothVault 规范别名迁移；3. 解包前拒绝绝对路径、路径穿越、链接和特殊归档条目；4. 离线安装优先使用归档自带 node_modules，否则仅尝试本地 npm 缓存；5. 插件进程成功启动时持久化最近运行时间并向 SlothVault 传递经验证的主命令路径；6. 异步执行 npm/tar 等外部命令，并为在线请求注入网络配置和平台选择。
 * @dependencies Node: child_process.spawn/fs/https/os/path, Storage: ../registry.js, Settings: ../settings.js, Network: ./network-helper.js, Data: ../official-plugins.json, I18N: ../i18n.js, System: ./system-environment.js
...
export function normalizePluginAlias(alias) {
    return deprecatedPluginAliases.get(alias) || alias;
}
...
function moveFileIfCanonicalMissing(legacyPath, canonicalPath) {
...
export function migrateSlothvaultPluginAlias() {
    ...
    const legacyConfigPath = getPluginConfigPath(legacyAlias);
    const canonicalConfigPath = getPluginConfigPath(canonicalAlias);
    const legacyHistoryPath = path.join(getSlothToolHome(), 'data', legacyAlias, 'history.json');
    const canonicalHistoryPath = path.join(getSlothToolHome(), 'data', canonicalAlias, 'history.json');
    ...
    result.profile = moveFileIfCanonicalMissing(legacyConfigPath, canonicalConfigPath);
    result.history = moveFileIfCanonicalMissing(legacyHistoryPath, canonicalHistoryPath);
    return result;
}
...
function preparePluginAlias(alias, reporter) {
    ...
    if (canonicalAlias === 'slothvault') {
        ...
        if (migration.plugin === 'conflict' || migration.profile === 'conflict' || migration.history === 'conflict') {
...
async function extractTarballSafely(archivePath, extractDir) {
...
async function stageOfficialPluginRelease(pluginMeta, releaseInfo, reporter) {
    ...
    try {
        ...
        await runCommand(getNpmBinary(), ['install', '--omit=dev'], {
            cwd: buildDir,
            env: proxyCommandEnv
        });
...
export function uninstallPlugin(alias, options = {}) {
    ...
    try {
        removePath(getPluginDir(canonicalAlias));
        removePath(getPluginConfigPath(canonicalAlias));
        registry.removePlugin(canonicalAlias);
        report(reporter, 'success', t('uninstall.success', {alias: canonicalAlias}));
        return {status: 'uninstalled', alias: canonicalAlias};
    } catch (error) {
        throw createCliError(t('uninstall.failed', {alias: canonicalAlias, reason: error.message}));
    }
}
