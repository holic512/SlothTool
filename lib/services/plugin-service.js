 ...
 * @description 提供官方插件在线/离线安装、离线自包含归档、更新、卸载、运行、最近运行记录、自更新和全量数据删除等无 UI 生命周期能力，并统一接入代理与 GitHub 镜像配置。
 * @logic 1. 用 reporter 事件输出进度；2. 统一处理 GitHub Release、离线归档与遗留 npm 来源插件；3. 解包前拒绝绝对路径、路径穿越、链接和特殊归档条目；4. 离线安装优先使用归档自带 node_modules，否则仅尝试本地 npm 缓存；5. 插件进程成功启动时持久化最近运行时间；6. 异步执行 npm/tar 等外部命令，并为在线请求注入网络配置和平台选择。
...
export function runInstalledPlugin(alias, args = [], options = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on('spawn', () => {
            try {
                lastRunAt = recordPluginRun(alias);
            } catch {
                lastRunAt = null;
            }
        });
...
export async function installPluginFromArchive(alias, archivePath, options = {}) {
    ...
    try {
        ...
        const entry = await installer(alias, officialPlugin, archivePath, reporter);
        report(reporter, 'success', t('install.success', {alias}));
        report(reporter, 'info', t('install.runHint', {alias}));
        return {status: 'installed', alias, plugin: entry, sourceType: 'offline-archive'};
    } catch (error) {
...
export async function createOfflinePluginBundle(alias, outputPath, options = {}) {
    ...
    const pluginRoot = getPluginDir(alias);
    ...
    if (dependencies.length > 0 && !fs.existsSync(path.join(pluginRoot, 'node_modules'))) {
        throw createCliError(t('bundle.dependenciesMissing', {alias}));
    }

    const defaultName = `${alias}-${pkg.version || 'bundle'}-offline.tgz`;
    ...
    try {
        ...
        return {alias, outputPath: destination, packageName: pkg.name, version: pkg.version};
    } finally {
        removePath(tempRoot);
    }
}
