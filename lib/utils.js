 ...
 * @logic 1. 统一生成 ~/.pipker/slothtool 用户目录与插件目录路径；2. 提供目录确保与安全删除；3. 判断当前终端是否适合启动 TUI。
 * @dependencies Node: fs/os/path
 * @index_tags 路径工具, pipker, 数据目录, TUI终端检测, 文件系统辅助
 * @author holic512
 */
...
export function getSlothToolHome() {
    return path.join(getPipkerHome(), 'slothtool');
}

export function getPipkerHome() {
    return path.join(os.homedir(), '.pipker');
}
