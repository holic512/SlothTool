...
import {createCliError, isDeprecatedPluginAlias, normalizePluginAlias, runInstalledPlugin} from '../services/plugin-service.js';
...
export default async function run(args, options = {}) {
    ...
    if (isDeprecatedPluginAlias(pluginAlias)) {
        console.error(t('pluginAliases.deprecated', {
            legacy: pluginAlias,
            canonical: normalizePluginAlias(pluginAlias)
        }));
    }
