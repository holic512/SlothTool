 ...
 * @description 启动默认全屏 Ink TUI，并在退出后处理需要落回 shell 的插件启动动作与最近运行选择恢复。
 * @logic 1. 在交互式终端中启动根 TUI；2. 接收 TUI 退出动作；3. 对 run-plugin 动作调用底层运行 service；4. 运行记录成功后让返回页面聚焦最近运行列表首项。
...
function relaunchRootCommand() {
    const childEnv = {
        ...process.env
    };

    if (
        process.env.SLOTHTOOL_TUI_TEST_ACTION === 'restart-self' ||
        process.env.SLOTHTOOL_TUI_TEST_ACTION === 'self-update-restart'
    ) {
...
function focusMostRecentPlugin(uiState) {
    if (!uiState) {
        return null;
    }
    ...
    while (true) {
        ...
        if (exitAction?.type === 'run-plugin' && exitAction.alias) {
            ...
            try {
                ...
                if (result?.lastRunAt) {
                    nextTuiState = focusMostRecentPlugin(nextTuiState);
                }
