...
function buildDividerLine() {
    const terminalWidth = Math.max(24, (process.stdout.columns || 80) - 4);
...
function LogsPanel({logs}) {
    return h(
        ...
        logs.length === 0
            ? h(Text, {dimColor: true}, t('tui.logsEmpty'))
            : null,
        ...logs.map((log, index) =>
            h(Text, {
...
function RootTuiApp({onExit}) {
    ...
    if (currentTab === 'home') {
        leftPane = h(
            ...
            ...HOME_ART.map(line => h(Text, {key: line, color: 'cyan', bold: true}, line)),
