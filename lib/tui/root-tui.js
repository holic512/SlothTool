 ...
 * @project SlothTool
 * @module Core CLI / TUI
 * @description 提供 SlothTool 默认全屏 Ink 界面，负责页面导航、最近运行插件排序、实时状态栏、两阶段更新、独立卸载页和插件返回后的状态恢复。
 * @logic 1. 使用 alternateScreen 渲染独立全屏页面；2. 在首页、运行、安装、更新、卸载、设置页之间切换；3. 运行页按最近运行时间排序，卸载页保留稳定插件顺序；4. 任务 reporter 实时驱动底部状态栏；5. 退出运行插件时返回 UI 快照给 interactive 命令恢复状态。
