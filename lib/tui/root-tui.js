 ...
 * @project SlothTool
 * @module Core CLI / TUI
 * @description 提供 SlothTool 默认全屏 Ink 界面，负责页面导航、底部提示、两阶段更新、独立卸载页和插件返回后的状态恢复。
 * @logic 1. 使用 alternateScreen 渲染独立全屏页面；2. 在首页、运行、安装、更新、卸载、设置页之间切换；3. 列表页采用上方列表与底部 tip 详情；4. 退出运行插件时返回 UI 快照给 interactive 命令恢复状态。
...
function RootTuiApp({onExit, initialState, initialStatus}) {
    ...
    return h(
        Box,
        {flexDirection: 'column', height: getViewportHeight(), paddingX: 1, paddingY: 1},
        h(RootHeader, {currentTab}),
