...
`slothtool slothvault` 的全屏管理页可通过 Tab 进入“部署”，用 ↑↓ 选择安装、状态、检查更新、更新、启停、Nginx、HTTPS 或证书续约，按 Enter 执行。按 `e` 编辑部署目录，按 `m` 切换系统级或官方 Docker Nginx 模式，Docker 模式按 `c` 输入容器名。会修改宿主机的操作需要在 TUI 中确认；随后内置安装器接管终端完成具体配置与确认，结束后返回调用方终端或根管理 TUI。需要管理员权限时以保留用户 HOME 的 `sudo env HOME="$HOME" "$(command -v slothtool)" slothvault` 启动管理页。
