...
test('root TUI zh chrome is localized', () => {
    ...
    assert.equal(messages.zh.tui.tabs.uninstall, '卸载');
    ...
    assert.equal(messages.zh.tui.update.listTitle, '更新中心');
    assert.equal(messages.zh.tui.update.fields.current, '当前版本');
    assert.equal(messages.zh.tui.uninstall.listTitle, '卸载目标');
    assert.equal(messages.zh.tui.uninstall.badges.danger, '高风险');
    assert.equal(messages.zh.tui.settings.listTitle, '设置项');
    assert.equal(messages.zh.tui.settings.fields.next, '执行后');
    assert.equal(messages.zh.tui.uninstall.action, 'Enter 打开确认并执行卸载');
    assert.equal(messages.zh.tui.tip, undefined);
...
test('root TUI en chrome stays explicit', () => {
    ...
    assert.equal(messages.en.tui.tabs.uninstall, 'Uninstall');
    ...
    assert.equal(messages.en.tui.update.listTitle, 'Update center');
    assert.equal(messages.en.tui.update.fields.current, 'Current');
    assert.equal(messages.en.tui.uninstall.listTitle, 'Removal targets');
    assert.equal(messages.en.tui.uninstall.badges.danger, 'High risk');
    assert.equal(messages.en.tui.settings.listTitle, 'Settings');
    assert.equal(messages.en.tui.settings.fields.next, 'After Enter');
    assert.equal(messages.en.tui.uninstall.action, 'Press Enter to confirm and run the uninstall action');
    assert.equal(messages.en.tui.tip, undefined);
